...
API_NAME=book-outlet
API_URL=https://localhost:8443/book-outlet
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST -H "Content-Type: application/json" -d "{\"name\": \"$API_NAME\", \"identifier\": \"$API_URL\", \"scopes\": [{ \"value\": \"book:read\", \"description\": \"Read book information\"}, {\"value\": \"book:write\", \"description\": \"Update book information\"}]}" https://$TENANT/api/v2/resource-servers | jq .
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$READER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"book:read\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$WRITER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"book:read\",\"book:write\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
